...
export const FAQ: Record<Lang, QA[]> = {
  en: [
    ...
    { q: 'What data does the hook send?',
      a: 'Nothing but the fact that a session started. The server records the current UTC hour and weekday; no prompts, code, file names, user IDs or IP addresses are stored. See <a href="/about/">privacy</a>.' },
  ...
  ru: [
    ...
    { q: 'Какие данные отправляет hook?',
      a: 'Только факт начала сессии. Сервер записывает текущий час и день недели по UTC; промпты, код, имена файлов, идентификаторы пользователей и IP-адреса не хранятся. Подробнее — в разделе <a href="/ru/about/">о приватности</a>.' },
