...
export const GET: APIRoute = async ({ props }) => {
  ...
  const tree = h('div', {
    ...
    background: '#faf9f5', padding: '64px 72px', fontFamily: 'Sans, SansCyr', color: '#141413',
  }, [
    h('div', { display: 'flex', alignItems: 'center', gap: 14, fontSize: 30 }, [
      ...
      h('div', { color: '#73726c', fontSize: 24, marginLeft: 12 }, lang === 'ru' ? 'пиковые часы Claude Code' : 'Claude Code peak hours'),
    ]),
    h('div', { display: 'flex', fontFamily: 'Serif, SerifCyr', fontSize: meta.title[lang].length > 48 ? 58 : 68, lineHeight: 1.12, letterSpacing: -1, maxWidth: 1050 }, meta.title[lang]),
